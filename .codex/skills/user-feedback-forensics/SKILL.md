...
- This skill is stored in the repository at `.codex/skills/user-feedback-forensics/SKILL.md`. If the runtime-provided skill list does not include `user-feedback-forensics`, still read and follow this file directly when the user asks for it.
- The `App Version` shown in GitHub feedback issue bodies is currently generated by the API from `env.API_COMMIT_ID` in `api/src/v1/feedback/feedback.service.ts`. It is the backend commit id, not the client app version or client commit id.
- Do not infer App Store / production / preview / development status from the GitHub issue body's `App Version` alone.
