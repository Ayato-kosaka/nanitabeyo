...
#1761 でグリッド（一覧）もフィードへ寄せたので、シート版のシナリオ
（`my-dishes-own-review.mjs`）は削除した。グリッドの経路は
`app-expo/__tests__/myDishesGridFeedVertical.test.tsx` が
«押す → フィードのページ → クチコミが出る» まで通しで見ている。
