...
オーナー指摘（2026-08-25）:
  ...
  → 追加指示:「セクションタイトルをつけて欲しいんじゃなくて、ページ遷移をするように」
...
async function shootScheme(scheme) {
	return record({
		...
		flow: async (page, shot) => {
			...
			await page.getByTestId("settings-device-settings").waitFor({ state: "visible", timeout: 10000 });
			await page.getByTestId("settings-about").waitFor({ state: "visible", timeout: 10000 });

			/*
			«移した» のであって «両方に置いた» のではないことを、撮る前に機械的に確かめる。
			設定画面に規約やテーマが残っていると、並べ替えたのに導線が二重化した状態になる。
			*/
			for (const gone of ["settings-terms", "settings-privacy", "settings-theme-selector"]) {
				if ((await page.getByTestId(gone).count()) > 0) {
					throw new Error(`設定画面に «${gone}» が残っている。#1583 は移設であって複製ではない`);
				}
			}
			...
			// ── 3. なに食べよについて ──
			await page.goto(`${BASE}/ja-JP/profile/settings`, { waitUntil: "domcontentloaded" });
			await page.getByTestId("settings-about").waitFor({ state: "visible", timeout: 15000 });
			...
			// バージョン行。押せない行なので testID でしか捕まえられない
...
await writeNote("settings1583", [
	"# #1583 設定の構成をゼロベースで見直す（1 画面 → 3 画面）",
	"",
	"設定 → 端末設定 / なに食べよについて の **ページ遷移**にした（オーナー指示）。",
	"",
	"- 01-settings … 設定。ページへ送る 2 行があり、規約・テーマは残っていない",
	"- 02-device-settings … 端末設定。表示テーマがここへ移った（**本命**）",
	"- 03-about … なに食べよについて。規約 4 行とバージョン（**本命**）",
	"- 04-about-version … バージョン行を画面内に入れた 1 枚",
	"",
