...
- 設計承認の推測を行わない。**デフォルトブランチへのマージは、仕様判断を含まない PR に限りリーダーが自分で行う**（2026-09-23 オーナー合意。それ以前は «一切行わない» だったが、オーナーを 10 本のマージボタンにする事故が起きたため改めた）。仕様・UX が変わる PR は、エビデンスを添えてオーナーへ出し、マージはオーナーの判断を待つ。
...
#### ⚠️ Artifact へのリンクはエビデンスにならない（保持期間で消える）

2026-09-25 に実測した。[#1780](https://github.com/Ayato-kosaka/nanitabeyo/issues/1780) は条件
「web・ネイティブ両方のエビデンスで確認している」を **Detox の run URL + Artifact 内のパス**で満たしたと書いて閉じた。
その Artifact は **09-12 と 09-17 に期限切れ**になり、**いまスクリーンショットは 1 枚も取り出せない**。
クロージングコメントの文章だけが残っている。

| Artifact | 保持 | エビデンスに使えるか |
| --- | ---: | --- |
| `e2e-mobile/screenshots/**`（`UI_CATALOG.md` 付き） | 14 日 | **こちらを使う。** 見せるために作られている |
| `detox-report-android`（`e2e-mobile/artifacts/`） | **7 日** | ⛔ **使わない。** `retention-days: 7` は «artifact には長期資格情報が載りうる» という理由で**意図的に短くしてある**（#1030 レビュー B-2） |

- **`detox-report-android` を名指ししてエビデンスとしない。** 短命なうえ、資格情報が載りうる前提の置き場である
- **14 日でも «恒久» ではない。** 閉じた条件の根拠として後から引くつもりのものは、
  `evidence-collect.yml` で公開するか `docs/evidence/<Issue番号>/` へコミットして**リポジトリに残す**
- run URL は補足として貼ってよい（ログは残る）。**ただし画像の代わりにはならない**
