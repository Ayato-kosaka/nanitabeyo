...
さらに厄介な事故として、**リーダーの実行環境が外向きの本文を書き換え、画像埋め込みがコードスパンへ中和されることがある**。エージェント環境には、画像URL経由の情報持ち出しを防ぐため送信内容の `![...](...)` を無効化するものがある。この場合、

- リーダー側で正しい記法を組み立てていても、GitHub 上では `` `` `` で囲まれた文字列として保存される
- 壊れる位置は一定しない（`!` の位置とURLの先頭のどちらにも入り得る）
- リンク記法（`!` なし）や生のURLは通るため、「リンクは出るのに画像だけ出ない」という形で現れる
...
4. ワーカーにも**投稿後の再取得による検証を必須**として指示し、壊れていたら「壊れている事実と実際の文字列」を報告させる。成功と書かせない。
