...
### テストの前提を「人への依頼」で表現しない

「TEST_USER に料理カテゴリを1件保存しておいてください」のような *依頼* をfail-loudメッセージで
書くのは、**環境に前提を置いたテスト**であり必ず壊れる。誰かがその状態を崩すだけで
**コード変更ゼロでCIが赤くなる**（実際に赤くなった）。新しいdev環境でも最初から赤い。

前提は**テスト自身が作る**。スタブできない環境（Detoxにネットワークスタブが無い）では、
APIやDBへ直接シードしてよい。ただし:

- 書くのは**前提が満たされていないときだけ**（2回目以降は書き込みゼロ）
- **後始末をしない方が正しい場合がある。** Android/iOSジョブは同時に走るため、
  片方の `afterAll` がもう片方の使用中の行を消す競合が起きる。作りたいのがrunごとの
  一時データではなく**恒常的な前提**なら、消さずに収束させる（`utils/savedDishCategory.ts`）
- UNIQUE制約違反は「既に前提が満たされている」と読み替えて成功扱いにする

### iOS の `toBeVisible` は「ソフトキーボード」で落ちる

Detoxの `toBeVisible` は **面積の75%以上が可視**を要求する。iOSのソフトキーボードは
画面の下半分を占有するため、その裏に来た要素は永久に条件を満たさず
`Unable to scroll down ... does not pass visibility percent threshold (75)` になる。

⚠️ `scripts/setup-ios-simulator.sh` の `ConnectHardwareKeyboard` は
**simctl起動のシミュレータには効かない**（スクリプト自身のコメントに実測付きで明記）。
ログに `✅ ConnectHardwareKeyboard = 1` が出ていても、キーボードは出ている。

さらに厄介なのは、**閉じてもアプリ側が非同期に開き直す**こと。
`LocationAutocomplete` は現在地取得に失敗したとき手入力へ誘導するため `focus()` を呼ぶ（#932）。
CIでは位置情報が取れないので、チュートリアル完了の数秒後にこれが必ず発火する。
**単発の `dismissKeyboard()` では、閉じてから探す間に開き直されて詰む。**

対処は次のいずれか。同じ `threshold (75)` で3回落として辿り着いた結論なので、
安易に単発の閉じ方へ戻さないこと（run 31607285195 / 31624910689 / 31644130515）。

- 「閉じる → 探す」を数回リトライする（`SearchScreen.openAdvancedFilters`）
- 可視ではなく**存在**で見る（`tapWhenPresent` / `waitUntilExists`）。画面下端に固定される
  検索FABは元からこの扱い（`SearchScreen.submit`）

### 過渡状態を観測点にしない

「一瞬だけ出る画面」をアサーションの対象にすると、Detoxは**アプリがidleになってから**
matcherを評価するため原理的に取り逃がす。共有リンクの解決画面（`share-link-resolver`）で
2回赤くした。**留まる状態**（着地先の画面）を観測点にすること。

観測点を変えると「捨てられた」と「受け取って失敗した」を区別できなくなる場合は、
テストデータを実在させて着地先そのものを変える（`utils/shareLink.ts` は実在の共有リンクを作る）。
