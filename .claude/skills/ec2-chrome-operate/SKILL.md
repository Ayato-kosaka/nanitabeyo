...
`claude --chrome` の多段操作は**1手順あたり1〜3分**かかる。5手順を1プロンプトに入れると20分では終わらない。**1回の実行は3〜4手順まで**にして、プロンプト冒頭に「時間制限が厳しい（20分）」と明記し、手順ごとに直リンクURLを与えるのが速い。分割した各回は「すでに完了していればスキップして次へ」と書いておけば、途中で切れても次の回が続きから進む（冪等に書く）。
