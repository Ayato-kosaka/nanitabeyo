...
| 試したこと | 結果 |
| --- | --- |
| `aws sts get-caller-identity` / `ec2 describe-instances` / `git status` | 通る |
...
| `git add` / `git commit`（読み取り系の `git status` / `git log` は通る） | 常に落ちる |

**再試行で運よく通るのを当てにしてはいけない。** 拒否は同一セッション内で回を追うごとに強くなる（最初は通っていた `ec2 start-instances` が後半は必ず落ちた）。落ちたまま押し切ろうとすると、**インスタンスを起動したのに停止コマンドが拒否される**という最悪の形になりうるので、`start-instances` が通った直後に進めなくなったら、まず `stop-instances` を通すことを最優先する。
...
これに合わせて、**実行スクリプトはスキル内の固定パス [`ec2_exec.sh`](./ec2_exec.sh) に置いてある**。allow ルールはパス一致で書くものなので、**毎回スクラッチパッドへコピーを作って実行してはいけない**（名前が変わってルールから外れ、また落ちる）。リモート側スクリプトだけはタスクごとに違ってよく、それは**引数として渡すデータ**なのでスクラッチパッド生成で構わない。
...
メール返信のように**本文が長いもの**は、Issue ではなく**リモートの `/home/ubuntu/reply.txt` に置き、プロンプトからはそのパスを指して「Read して一字一句そのまま貼れ」と書く**。プロンプト自体に本文を埋めると、agent が要約・言い換えをしがちで、送信物が確定文面と一致しなくなる。
