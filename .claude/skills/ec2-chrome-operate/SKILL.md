...
### 実行時間を縮める（2026-08-14 の実測ベース）

#1331 は6ブート・全体で約60分かかった。内訳を測ると、**待ち時間の大半は「本命の操作」ではなく1ブートごとの固定コスト**だった。

| 区間 | 実測 | 備考 |
| --- | ---: | --- |
| start → SSM Online | 20〜30秒 | |
| Chrome 起動の固定 `sleep 60` | 60秒 | 削ると拡張が繋がらない。削らないこと |
| 接続プローブ `claude -p` 1回 | 約90秒 | 全5回とも **probe 1 で接続**した |
| 本命の `claude --chrome -p` | 2〜10分 | 内容次第 |
| stop → stopped 確認 | 60〜90秒 | |

つまり**本命以外に毎回およそ4分**かかる。効いた順に:

1. **複数の手順を1ブートに詰める。ただし1つの `claude -p` にではなく、`claude -p` を順番に複数回呼ぶ。** Chrome 起動とプローブ（約2.5分）はブート内で1回で済む一方、`-p` を分ければ各回の報告が独立して残り、コンテキストも毎回リセットされる。「1プロンプトに詰めすぎない」と「1ブートに詰める」は両立する。
2. **`claude` を挟まない診断は `ec2_exec.sh` に素のシェルスクリプトを渡す。** `xdotool` の有無を見た回は**コマンド実行30秒・1周2分弱**で終わった。「入っているか分からないもの」を本命プロンプトの中で調べさせると、失敗したときに丸ごと1回分（10分以上）を失う。
3. **プロンプトに直リンクURLを書く。** コンソールをメニューから辿らせると数分違う。今回は全手順に `https://console.cloud.google.com/...?project=...` を直書きした。
4. **「すでに完了しているかもしれないので確認してスキップ」と書く。** 途中で切れた回のやり直しが、全5手順の確認込みで**155秒**で終わった。
5. **スクショと `Read` の往復を必要最小限にする。** 証跡回は78ターン・キャッシュ読み 560万トークンまで膨らんだ。画面全体を撮って読むより、`zoom` で必要な領域だけ見るほうが速い。
6. **並列化はできない。** インスタンスは1台、Chrome とブリッジ接続も1つなので、2本同時に流すことは前提から無理。縮めるならブート数を減らすしかない。
