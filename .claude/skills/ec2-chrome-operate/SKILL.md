...
3. [`RUNBOOK.md`](./RUNBOOK.md) の**リモート側スクリプト**（Chrome 起動 → 拡張の接続待ち → 本命プロンプト）を、プロンプトを**クォート付きヒアドキュメントでそのまま埋めて**スクラッチパッドに生成する（base64 は使わない。上の「権限クラシファイア」節）。生成したら

   ```
   bash .claude/skills/ec2-chrome-operate/ec2_exec.sh <生成したリモートスクリプト> 2400
   ```

   を**自分の Bash ツールで `run_in_background: true`** で走らせる。**必ずこの固定パスから実行すること**（allow ルールがこのパスに紐づいている）。`nohup ... &` を自分で書くと classifier に止められる。フォアグラウンドは Bash の上限600秒を超えるので不可。
