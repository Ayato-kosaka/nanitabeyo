 ...
 * | `パイナップルジュース` | `パイ` → パイ | 同上に加え、**カタカナの右境界**（`パイ` の直後が `ナ`）で不一致 |
 * | `アラフォー女子会` | `フォー` → フォー | 左の切れ端が 2 文字 ＝ 語を途中で切っている（`KATAKANA_MODIFIER_MIN_LENGTH`） |
 ...
 * 要点は「ラテン文字は語の途中に埋もれない」「カタカナは**右**に伸びない（`フォー` ⊄ `フォーク`）」で、
 * **ひらがなと漢字には境界を課さない**（課すと `おいしいうどん` `絶品味噌ラーメン` を落とすため）。
 * カタカナの**左**は、表記が 3 文字以上で、かつ直前のカタカナの並びも 3 文字以上なら伸ばしてよい
 * （`チーズケーキ` ⊂ `バスクチーズケーキ`）。日本語のカタカナ複合語は主要部が右端に来るので、
 * 左に付くものは上位語の具体化にしかならない。直前の長さを見るのは「語を途中で切った」ものを
 * 落とすため（`アド|バイス` `アラ|フォー`）。
 *
