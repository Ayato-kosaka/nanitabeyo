...
| Issue | 内容 |
| --- | --- |
| #1366 | `app-expo` の lint が CI に無く、既存 48 errors で落ちる |
| #1367 | `toInAppPath` が `/[locale]/auth/*` を落とす |
| #1374 | ネイティブのディープリンクで `next` が二重エンコードされる |
| #1387 | プロフィール編集で取得失敗時にスピナーが固着する |
| — | 法務文書の実体が ja-JP / en-US の 2 ロケールしか無く、sitemap の 32 URL のうち 24 本が英語本文の複製（`lib/seo/sitemap.ts` に記録） |
...
3. **`/legal/[doc]` の `expo export` 成果物**の確認（静的書き出しに載ること）
