...
## ⚠️ `--update-env-vars` だけでは効かない（このサービスは特にそう）

**入れたことと、効いていることは別である。**

`api-deploy.yml` は昇格時に `--to-revisions <REV>=100` で**トラフィックを名指しで固定**する
（#1323 のスモークゲートのため）。固定されたサービスでは、あとから
`gcloud run services update --update-env-vars` で作られたリビジョンは **0% のまま**になる。
つまり **env を入れても、次に api-deploy が走るまで効かない。**

実測（#843 / Maps Embed の鍵、2026-09-04〜05・すべて UTC）:

| 時刻          | 出来事                                                      | `POST /v1/maps/embed-token` |
| ------------- | ----------------------------------------------------------- | --------------------------- |
| 09-04 12:28   | `--update-env-vars` で鍵を追加（新リビジョンが 1 本できる） |                             |
| 09-04 12:48   | e2e で叩く                                                  | **503**（鍵が見えていない） |
| 09-04 18:29〜 | api-deploy が走り、env を引き継いだ新リビジョンが 100% へ   |                             |
| 09-05 15:57   | e2e で叩く                                                  | **201** ✅                  |

⚠️ **`/livez` が 200 でも確認にならない。** サービス URL は固定された（= 古い）リビジョンへ
流れるので、**鍵が入っていないほうが 200 を返す**。実際 09-04 の設定報告は `/livez` 200 を
根拠にしていたが、その 20 分後に 503 だった。**起動確認と機能確認を混同しないこと。**
