...
| 7   | ゲスト向けのログイン導線                    | 匿名ユーザーにログイン CTA が出て、**ログインモーダルが開く**（Google/Apple ボタンの表示まで）                        | `e2e-web/tests/profile/login-modal.spec.ts` ほか / `e2e-mobile/tests/profile/login-modal.test.ts`・`tests/review/review-guest.test.ts` |
