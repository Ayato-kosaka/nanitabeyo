...
| | 状況 |
| --- | --- |
| メモリ | 埋め込みセルを 5 本並べたところ Android が `lowmemorykiller` でアプリを殺した（クラッシュではなくプロセス消滅なので、クラッシュバッファには何も残らない）。セルごとの使用量を run のログへ残すようにした（[`e2e-mobile/utils/memoryProbe.ts`](../../e2e-mobile/utils/memoryProbe.ts)）。**直す前に、まず見えるようにした段階** |
