...
| #1130 | SafeArea | 検討中 | 検討中 |
| #1133 | 地点検索（許可ダイアログ以外） | 実装中 | 実装中 |
...
- [ ] **モーダル内の余白タップで閉じなくなったことが不自然でないか**（これは主観評価なので E2E で判断できません）
...
| #281 | **Search Console のプロパティを `app.nanitabeyo.net` へ切り替えて sitemap を送信** | Search Console にアクセスできない。**#721 は解決済みなので送信して良い** |
| #721 | (a) 共有プレビューの方針決定（SSR / 事前レンダ / 専用 URL。**推奨は専用の静的 URL**） | アーキテクチャの判断が要る |
