 ...
 *
 * 対応画面: app-expo/app/[locale]/(tabs)/profile/saved-topic-location.tsx
 *          （中身は features/profile/containers/SavedTopicLocationSearch.tsx →
 ...
 * このファイルの matcher も変わらないが、**到達判定には画面ヘッダー（`saved-topic-location-screen-title`）
...
export class SavedTopicLocationSearchScreen {
	...
	readonly screenTitle = by.id("saved-topic-location-screen-title");
	...
	async openLocationSearchFromFirstTopic(): Promise<void> {
		...
		if (!(await existsNow(this.topicCard(0)))) {
			throw new Error(
				[
					"保存した料理カテゴリが 0 件のため、地点検索画面を開けませんでした（save-topic-tab-item-0 が存在しない）。",
