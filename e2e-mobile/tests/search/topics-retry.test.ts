...
/**
 * 🔁 REL-03 (#1499): Topics 取得失敗時の「その場で再試行」（ネイティブ / Tier 2）
 *
 * 対応する e2e-web: e2e-web/tests/search/topics-retry.spec.ts
 *
 ...
 * `TopicsError.tsx` として Web/ネイティブで共通のコンポーネントであり、
 * `topics-error` / `topics-error-retry` の testID もプラットフォーム間で共通に付いている。
 ...
 */
describe("Topics 取得失敗時の再試行（実 API・#1499）", () => {
	...
	it("取得失敗時は再試行ボタンが表示され、押すと再取得へ遷移する（到達できた場合のみ）", async () => {
		const outcome = await topics.expectSettled();
