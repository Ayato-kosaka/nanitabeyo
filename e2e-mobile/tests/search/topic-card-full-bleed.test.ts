 ...
 *
 * 対応する e2e-web: e2e-web/tests/search/topic-card-full-bleed.spec.ts
 *
 * 背景: `useTopicCardSize()` は既定で左右16pxずつ(計32px)の余白を残した
 * `contentWidth - 32` を返す。topics.tsx の候補カルーセルだけは `fullBleed: true` を渡し、
 * 余白ゼロ(=画面幅そのまま)の値を使うよう変更した(app-expo/features/topics/hooks/useTopicCardSize.ts)。
 ...
 *
 * 観測点: `topics-tutorial-target-swipe`(app-expo/features/topics/components/TopicCard.tsx)。
 * `tutorialTargetRefs` は topics.tsx が isActiveCard のときにしか渡さないため、
 * この testID は**アクティブなカード1枚にだけ**存在する(前後の事前描画カードには付かない)。
 * TopicsScreen.ts には無い testID のため、この spec 専用にローカルで matcher を持つ。
 *
...
describe("候補カルーセルのカード幅とスナップ位置(#1212)", () => {
	...
	const activeSwipeArea = by.id("topics-tutorial-target-swipe");
	...
	beforeAll(async () => {
		...
		// tests/search/topics-flow.test.ts と同じ手順)
	...
	it("カード幅は画面幅いっぱいで、左右の余白が無い", async () => {
		...
		assert.ok(frame, "アクティブなカードの swipe area (topics-tutorial-target-swipe) が見つかりません");
		...
		assert.ok(
			...
			[
				...
				"  useTopicCardSize({ fullBleed: true }) が正しく渡っているか確認してください。",
			].join("\n"),
		);
	});
