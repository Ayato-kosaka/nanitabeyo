	...
	*/
	...
	it("取り込んだリールがフィードで自動再生され、再生できない投稿だけが導線へ縮退する", async () => {
		...
		*/
		...
		実測（run 33348354215 / commit 1d21d7cc / iOS）: TikTok が
		`timeout still_loading` を出したのは **前面に来てから 23.2 秒**だった
		（LOADING_GRACE_MS 20 秒 + 検出）。18 秒で切り上げていたので赤くなっていた。

		印が出た時点で抜けるので、この値を延ばしても **結論の出ないセルにしか効かない**
		（大半のセルは 2〜8 秒で抜ける）。
		*/
		const CELL_DWELL_MS = 26_000;
