 ...
 *（それは topics-group-vote-double-tap.test.ts が担保している）。#1507 では
 ...
 * **web 側でしか担保できない**（review-submit-loading.test.ts / topics-group-vote-double-tap.test.ts と
 * 同じ位置づけ）。
 *
 ...
 *（topics-group-vote-double-tap.test.ts と同じ制約）。
...
describeMutation("友達投票作成の再送耐性 @mutation", () => {
	...
	it("作成に失敗したあと押し直すと、投票結果画面が 1 枚だけ開く", async () => {
		...
		// #1205 のガード解除（hook の finally と topics.tsx の catch）が壊れると、ここから先へ進めなくなる
		await topics.expectHeaderVisible();
		await waitUntilVisible(topics.groupVoteButton);
