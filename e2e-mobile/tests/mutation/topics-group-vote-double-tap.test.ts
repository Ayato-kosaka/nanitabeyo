 ...
 *
 * e2e-web の tests/search/topics-group-vote-double-tap.spec.ts に対応する。
 *
 * ## 何を守っているか（#1205）
 ...
 * #1205 で `useCreateDishCategoryGroupVote` の `isCreatingRef` と、topics 画面の
 ...
 * n 回のタップを 1 アクションとして送る `multiTap` を使う（`TopicsScreen.openGroupVoteRapid`）。
 *
...
describeMutation("友達投票開始ボタンの連打耐性 @mutation", () => {
	...
	//        ガードの解除は topics.tsx の useFocusEffect が握っており、解除を消すとここで
	...
	it("友達投票開始ボタンを連打しても投票結果画面は 1 枚しか開かない", async () => {
		...
		await topics.openGroupVoteRapid(PRESS_COUNT);
