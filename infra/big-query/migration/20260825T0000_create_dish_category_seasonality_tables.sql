...
CREATE TABLE IF NOT EXISTS `food-scroll.wikidata_food_graph.dish_category_seasonality_curve` (
  ...
  index_value     FLOAT64 NOT NULL OPTIONS(description="月指数（全記事共通の季節成分を割り戻した後）。1.0 = 平常月。publish に使うのはこちら"),
  index_raw       FLOAT64 NOT NULL OPTIONS(description="割り戻す前の生の月指数。監査用。8月は全記事が一律 15% 沈むため index_value と乖離する"),
