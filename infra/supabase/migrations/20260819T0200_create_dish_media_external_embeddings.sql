...
CREATE TABLE IF NOT EXISTS dish_media_external_embeddings (
  ...
  CONSTRAINT dmee_provider_check
    CHECK (provider IN ('x','instagram','tiktok','threads','youtube')),
...
COMMENT ON COLUMN dish_media_external_embeddings.provider IS '埋め込み元 SNS（x / instagram / tiktok / threads / youtube）。provider 別の埋め込みコンポーネントを選ぶのに使う';
