...
## PR ルール（フロントエンド変更時のスクリーンショット添付）

### 対象

- `app-expo/` 配下の変更（UI/画面/コンポーネント/スタイル/ナビゲーション/文言/i18n）
- もしくは「画面表示に影響する可能性がある」変更（状態管理・表示条件・APIレスポンス表示など）

### 必須要件

- **PR本文にスクリーンショットを添付すること（必須）**
  - 変更前 / 変更後（可能なら両方）
  - 影響画面が複数ある場合は、主要な画面すべて
  - ダークモード・多言語など影響が大きい場合は代表例も添付

### 例外（スクショ免除できるケース）

- UIに一切影響しない変更（例：型定義のみ、コメントのみ、ビルド設定のみ など）
- ただし免除する場合は、PR本文に **「スクショ不要の理由」** を明記すること

### 推奨（あるとレビューが速い）

- 端末/OS（例：iPhone 15 / iOS 18, Android Pixel / Android 15, Web など）
- 画面名と操作手順（どこをどう変えたかが1分で再現できる情報）
