...
| **C** | Common Crawl で店舗公式サイトを読む | website を持つ店の **18.3%**。全店換算 **4.86%** — 4.3（15.17%）の一部を無料で置き換えるだけ | 無料 |
...
⚠️ **この 18.3% の分母は «website を持つ店» である。** 標本は FSQ で `website` が
埋まっている店から取った。FSQ の日本の飲食（営業中）1,177,934 店のうち website があるのは
312,559 店（26.5%）なので、**全店に換算すると 4.86%** にしかならない。

**4.3 の自前クロール（全店の 15.17%）を置き換えるものではなく、その一部を
«相手サーバに触れず・無料で» 取れるという話である。** n=60 なので幅も広い（±9.8pt）。
1 件は `embed.js` の誤検出、ページ取得に失敗したホストがあるため過小評価側でもある。
