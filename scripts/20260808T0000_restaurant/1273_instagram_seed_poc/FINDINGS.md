...
## 2026-09-05 — 柱1 の «採れていない 92.7%» の内訳と、1 コールあたりの枚数

### 1. 採れていない理由は «失敗» ではなく «一度も投げていない»

店 IG handle の在庫 40,911（37,719 店）に対し、投稿が 1 枚でもあるのは 6,296（15.4%）。
その内訳を `restaurant_pipeline_runs` の 4_2 実行履歴と `sns_post_raw` の handle 分布から実測した。

| 区分 | handle | 在庫比 |
| --- | ---: | ---: |
| business_discovery に **投げた** | 3,490 | 8.5% |
| ↳ うち投稿が取れた | 2,785 | （投げた分の 79.8%） |
| ↳ うち 0 枚（非ビジネス / 非公開 / 削除・code 110） | 705 | （投げた分の 20.2%） |
| **一度も投げていない** | **37,421** | **91.5%** |
| ↳ うち他経路（cc_wat / gourmet_media 等）で偶然採れた | 3,513 | |

**«クォータで落ちた» でも «handle が無効» でもない。単に流していない。**
4_2 の store_account 収集は 4 run しか無く、いずれも handle 昇順の先頭から始まって
`anjo_cat` / `cafe_washangare` / `coffeefiel` / `hanoji_gram` で止まっている（i〜z は未着手）。
原因は 2 つとも 4_2 のアカウント選択にあった。

1. `--account-run-id` が **単一 run_id** しか読めない。handle は 4 つの発見 run
   （sitecrawl 22,347 / fsq 15,967 / catalog 6,848 / 08-31 2,532）に分かれて入っており、
   **sitecrawl の 22,347 は 4_2 に一度も渡されたことが無い**。
2. «収集済み» の判定が出力 run の中に閉じていた。run を切り直すたびに同じ先頭から採り直す。

→ `--account-run-ids`（union）と `--skip-collected-scope any` で塞いだ。

### 2. IG business_discovery のクォータ実測

`restaurant_pipeline_runs` の 4_2 実行 6 本（処理アカウント数 ÷ 実時間）:

| run | アカウント | 実時間 | limit/acct | コール/acct | アカウント/時 |
| --- | ---: | ---: | ---: | ---: | ---: |
| 08-31 | 800 | 202 分 | 30 | 1 | 238 |
| 09-02-catalog | 1,200 | 340 分 | 50 | 1 | 212 |
| 09-02-fsq #1 | 1,200 | 332 分 | 50 | 1 | 217 |
| 09-02-fsq #2 | 1,200 | 269 分 | 50 | 1 | 268 |
| 09-03-storecap | 563 | 384 分 | 200 | 4 | **88** |
| 09-04-inflcap | 373 | 183 分 | 200 | 4 | 122 |

**~220 コール/時（アプリ単位）。`IG_TOKEN` は 1 本しか無いので並列シャードでも合算は増えない。**
未収集 119,590 ハンドルを全部回すと 544 時間（22.6 日）。段別では
A+B+C 15,795 件 = **71.8 時間（3.0 日）**、D 29,380 件 = 133 時間。

### 3. 1 アカウントあたりの枚数は «段» で違う

`media.limit(N)` は 1 コールで返るので **N<=50 なら 1 アカウント = 1 コール**、
N=200 だと 4 コール（上表 88/h 対 ~220/h がそれ）。そのうえで:

- **D 段（店アカウント）= 10 枚**。1 アカウント = 1 店なので要るのは «カテゴリが 1 つ以上決まる»
  ことだけ。柱1 の 2,104 店（投稿の 53.4% がカテゴリ付き）で
  3 枚 81.7% / 5 枚 87.2% / **10 枚 90.3%** / 20 枚 93.4% / 全 94.5 枚 93.4%。
  10 枚で満額の 96.7% を保ち、resolve の下流量は 1/9。
- **A/B/C 段（ご当地グルメ）= 50 枚**。1 アカウントが多数の店を回るので枚数を絞ると店ごと落ちる。
  実測で 198.6 投稿（4 コール）から 49.5 店 = 12.4 店/コール。**1 コールで取り切れる上限 50** が
  店/コールの最大点（51 枚目から 2 コール目に入る）。

## 2026-09-05 — 手作り influencer 一覧が唯一の «新しい店» の供給源（実測で確定）

同じ日に 2 つの run を同じ条件（同じ resolve・同じ 9_1 ビルド `sns-catalog-2026-09-05-p1`）で
比べた。基準は run `sns-catalog-2026-09-05c`（配信の異なり店 20,204 / KPI 134 基準 18,747）。

| run | 中身 | 収集アカウント | 配信店 | **新規の配信店** | 配信店/アカウント |
| --- | --- | ---: | ---: | ---: | ---: |
| `sns-2026-09-05-inflseed` | 手作り influencer 一覧（`sns_influencer_seed_handles.txt`） | **8** | 611 | **446** | **76.38** |
| `sns-2026-09-05-storefill` | 店アカウント（seed 有）ほか | 108 | 178 | **0** | 1.65 |

**店アカウントは 108 件収集して新規の配信店が 0 件だった。** 配信側（9_1）は
`discovery_seed_place_id` を信用して店を立てるので、店アカウントの投稿を採っても
**その店は既に配信カタログに載っている**。柱1 の «店アカウントを採り切る» は、
投稿は増えるが **配信の異なり店は増えない**。

逆に手作り一覧の influencer は **8 アカウントで新規 446 店**。予測（33.6 店/アカウント）を
上回る 76.4 店/アカウントだった。

### 供給が止まっていた原因は seed ファイルの取り込み漏れ

`sns_influencer_seed_handles.txt` は 741 件あるのに `sns_source_account` には 413 件しか
入っていなかった。`4_1 --source influencer_list` を流し直したら **328 件が新規**に入り、
未収集は 341 件になった（実測。予測 328 と一致）。**seed ファイルを足したら 4_1 を流す**、
が抜けていた。

### コスト（IG business_discovery ~200 コール/時・`IG_TOKEN` は 1 本）

influencer は 1 アカウント ≒ 310 投稿 ≒ 7 コール（`--limit-per-account 400`）。
未収集 341 件 × 7 ≒ 2,400 コール ＝ **約 13 時間**。実測の進みは 2 シャードで 22 アカウント/時
（≒ 154 コール/時）で、これはアプリ単位の枠そのもの。**シャードを増やしても合算は増えない。**
