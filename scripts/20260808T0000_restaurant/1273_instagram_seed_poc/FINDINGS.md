...
| --- | ---: | ---: | ---: | ---: |
| A 人が選んだ `influencer_list` | 377 | **36** | **31.88** | 1,148 |
| B 店アカウント（seed 有・非 curated） | 6,387 | **34,521** | **0.542** | 18,710 |
| C 地域語のみ（非 curated） | 4,132 | 13,089 | 0.145 | 1,898 |
| D 料理語のみ（非 curated） | 580 | 1,245 | 0.121 | 151 |
| E その他（非 curated） | 33,038 | 75,130 | 0.056 | 4,207 |

**順は A → B → それ以外。** A は **36 件**しか無いが 1 コールあたり B の **59 倍**なので必ず先に回す。
A を使い切った後の本体は **B（店アカウント）＝ 柱1 が既に進めている並び**である。

効果は正直に言って小さい: 15,768 コールで **8,546 → 9,675 店（+13%）**。その差は
**ほぼ全部 «A の 36 件を先に回す» ぶん**（36 コールで 1,148 店 対 20 店）。
つまりこの script の実質は «人が選んだ未収集 36 件を先頭に置く» ことに尽きる。
...
- よって初版の並べ替えは **逆効果**（15,768 コールで ~4,950 店 < 店アカウントの 8,546 店）。
