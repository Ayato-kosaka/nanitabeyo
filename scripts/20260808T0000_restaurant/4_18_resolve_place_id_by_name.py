...
## 全量の見積り — **律速は入力ではなく 1 日 75,000 request の枠**

BigQuery 実測（2026-09-05、`sample_name_sources.py` / 未確定の caption 付き投稿 361,703 件）:

| | 異なりキー | うち未問い合わせ |
| --- | ---: | ---: |
| 既定の書き方（pin + label + quoted） | 約 72,600 | 約 52,500 |
| 実装だけある書き方も足したとき（+ marker + bracket） | 81,790 | 62,039 |

**キーは足りていない側ではない。** 1 キーにつき request は 2 本なので、既定の書き方だけで
既に 105,000 request ≒ **1.4 日分**の枠が積み上がっている。したがって

- 書き方を増やすときは «増える量» ではなく «目視の確度» で決める（→ `DEFAULT_NAME_SOURCES`）。
  確度の低いキーを混ぜることは «その分だけ確かなキーを聞けない» と同義である
- 1 回の run は `--max-keys` で刻む。`--dry-run` が「今回の Google への request 見込み」を
  出すので、流す前にそれを見ること。済みのキーは `sns_name_place_lookup` を見て
  二度聞かないので、同じコマンドを日をまたいで繰り返すだけで全量が終わる

まだ当てていない入力（測ってあるが、上の理由で保留）:

- **収集座標からの市区町村**（`1841_place_id_by_name/measure_latlng_area.py`）。
  店名を採れても地点が分からず捨てている投稿が 47,464 件あり、うち座標を持つものは
  救える。ただし `discovery_area_lat/lng` は «探した場所» であって «店の場所» ではなく、
  キャプションに市区町村が書いてある 107,659 件で突き合わせると**一致率 84.4%**。
  枠が余るまでは、確度 95% の pin/quoted を先に消化する方が得である
