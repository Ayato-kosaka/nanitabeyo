...
class ProbeCache:
    ...
    def existing_keys(self) -> set[tuple[str, str]]:
        with self._lock:
            rows = self._connection.execute("SELECT seed_id, probe FROM probe").fetchall()
        return {(row[0], row[1]) for row in rows}

    def put_many(self, records: Sequence[tuple[str, str, SearchResult]]) -> None:
        payload = [
            (
                seed_id,
                probe,
            ...
            for seed_id, probe, result in records
        ]
        with self._lock:
            self._connection.executemany(
                "INSERT OR REPLACE INTO probe (seed_id, probe, status, ids, error)"
                " VALUES (?, ?, ?, ?, ?)",
                payload,
            )
            self._connection.commit()
...
def execute_probes(
    ...
    *,
    ...
    pending = [spec for spec in specs if (spec.seed_id, spec.probe) not in done]
    if not pending:
        return 0

    buffer: list[tuple[str, str, SearchResult]] = []
    ...
    def run(spec: ProbeSpec) -> None:
        ...
        with buffer_lock:
            buffer.append((spec.seed_id, spec.probe, result))
            completed += 1
