...
>
> **自分の dispatch でなくても消される。** 2026-08-31 には、別ブランチの作業が
> 2 本入っていたところへ投げたため、私の同期が 3 本目として消えた（job 0 本）。
> つまり «自分は 1 本ずつ投げている» だけでは足りない。dispatch したら
> **`.../runs/<id>/jobs` を引いて job が 1 本以上できたこと**を確かめる。
> 0 本なら流れていないので、キューが空くのを待って投げ直す。
...
⚠️ **ハードウェアは違う。絶対値は当てにならない。** 2026-09-01 の dev 実測で

| | ローカル | dev（Supabase） | 比 |
| --- | ---: | ---: | ---: |
| 合計 | 127.0秒 | 797秒 | **6.3×** |
| provenance UPDATE | 33.2秒 | 328.4秒 | 9.9× |
| synced_at | 34.8秒 | 389.3秒 | 11.2× |

**62 万行の «書き» は 10 倍遅い。** ここで見てよいのは «どの文が支配的か» と
«前後で何倍になったか» だけで、**«何分で終わるか» の予測には使えない**。
時間の予算を判断するときは 10 倍を見込むこと。
