...
2. `sns_name_place_lookup` の **`decision='city_box_unique_strict'` だけ**を辞書に使う
...
後入れした seed は `seed_source = 'name_place_lookup'` を立てる。4_18 の実測精度は
...
# まず 500 件だけ貼って、サンプルを人が見る
python3 4_21_link_name_place_to_posts.py --run-id sns-2026-09-10-namelink \
    --execute --limit 500 --sample-out out/name_place_link_sample.tsv
...
def build_link_rows(
    ...
    *,
    ...
    ここで守る 3 つ（どれも «決まらないなら書かない» 側へ倒す）:

    1. `decision='city_box_unique_strict'` **以外は使わない**。4_18 が捨てた理由
       （`city_box_not_unique` / `area_query_not_unique` / `probes_disagree` …）は
       «決まらなかった» であって «たぶんこれ» ではない。実測で緩めた場合の精度は
       79% → 64% まで落ちる（4_18 の module docstring）。
    2. 同じキーに **2 つ以上の place_id** が付いていたら使わない。判定を変えた再実行が
       同じ (店名, 市, 県) に別の place_id を書き残していることがあり、
       «新しい方» を選ぶ根拠はこちらには無い。
    3. 1 投稿に **2 店以上**当たったら書かない。
    ...
    stats = {"lookup_rows": 0, "non_strict": 0, "ambiguous_keys": 0, "matched_keys": 0,
             "ambiguous_posts": 0, "identity_route_posts": 0, "linked_posts": 0}

    # 1 + 2: キー → place_id（strict だけ・1 キー 1 店だけ）
    places_by_key: dict[tuple[str, str, str], set[str]] = {}
    versions: dict[tuple[str, str, str], str] = {}
    for row in lookup_rows:
        stats["lookup_rows"] += 1
        if row.get("decision") != resolver.DECISION_MATCHED:
            stats["non_strict"] += 1
            continue
        ...
        versions[key] = row.get("algorithm_version") or resolver.ALGORITHM_VERSION
    place_by_key = {}
...
def load_lookup_rows(pipeline: BigQueryPipeline, path: str | None) -> list[dict[str, Any]]:
    """4_18 の辞書を読む。**strict だけ**を SQL 側でも絞る（読む量を減らすため）。
    ...
    sql = LOOKUP_SQL.replace("__TABLE__", pipeline.table(TABLE_NAME_PLACE))
    rows = pipeline.execute(sql, [bigquery.ScalarQueryParameter(
        "decision", "STRING", resolver.DECISION_MATCHED)])
...
BACKFILL_SQL = """
  ...
  FROM (
    ...
    WHERE run_id = @run_id
    GROUP BY post_id
    -- 台帳は追記なので同じ投稿が複数行あり得る。2 店以上になっていたら書かない
...
def count_applied(pipeline: BigQueryPipeline, run_id: str) -> int:
    ...
    sql = (APPLIED_COUNT_SQL
           .replace("__RAW__", pipeline.table(TABLE_POST_RAW))
           .replace("__LINK__", pipeline.table(TABLE_NAME_PLACE_LINK)))
    row = next(iter(pipeline.execute(sql, [
        bigquery.ScalarQueryParameter("run_id", "STRING", run_id),
        bigquery.ScalarQueryParameter("seed_source", "STRING", SEED_SOURCE),
    ])))
    return int(row["applied"])
...
SAMPLE_SQL = """
  ...
  SELECT p.post_id, p.store_name, p.area_pref, p.area_city, p.name_source, p.google_place_id,
         r.canonical_url, r.discovery_route,
...
def write_sample(pipeline: BigQueryPipeline, run_id: str, path: Path, size: int) -> int:
    ...
    with path.open("w", encoding="utf-8", newline="") as stream:
        ...
        writer.writerow(["post_id", "store_name", "area_pref", "area_city", "name_source",
                         "google_place_id", "maps_url", "canonical_url", "discovery_route",
                         "caption_head"])
        for r in rows:
            writer.writerow([
                r["post_id"], r["store_name"], r["area_pref"], r["area_city"],
                r["name_source"] or "", r["google_place_id"],
...
def main() -> None:
    ...
    LOGGER.info("strict で確定しているキー %d 件（strict 以外 %d 行 / 1 キー 2 店 %d 件は不使用）",
    ...
    LOGGER.info("貼れる投稿 %d 件 / 異なり店 %d 件",
                len(rows), len({r["google_place_id"] for r in rows}))
    ...
    with pipeline.step(run_id, "4_21_link_name_place_to_posts",
        ...
        LOGGER.info("seed が入った投稿: %d 件（台帳 %d 件のうち。差は «選んでから書くまでに"
                    "別のジョブが埋めた» ぶん）", applied, len(rows))
        result["row_count"] = applied
