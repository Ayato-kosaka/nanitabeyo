...
| `7_1_build_coverage.py`（KPI 台帳） | 当てはまらない（**実データで判定**） | «数える側と配る側がずれる» 事故（#1846）と同じ形になり得るので当たり直した。7_1 は住所から都道府県を抜いて計上するが、海外 100,063 行のうち **住所に日本の都道府県を含む行は 0 行**なので region が NULL になり計上されない。近傍 1,500m からの補完（`nn`）で日本の region を拾い得る海外行も **2 行だけ**で、その 2 行は判定の既知の誤検知（実際は日本の店）だった。よって KPI は水増しされていない |
