...
# pipeline_common は import 時に `from google.cloud import bigquery` する。ここで検証するのは
# BQ に触れない純関数なので、本物が入っていなければ import を通すためだけのスタブを差し込む。
# **本物が入っているときはそのまま使う。** スタブで `google.cloud` を丸ごと置き換えると、
# 同じ実行に載る他のテスト（`google.cloud.storage` を読む 9_2）が import できなくなる。
try:  # noqa: SIM105
