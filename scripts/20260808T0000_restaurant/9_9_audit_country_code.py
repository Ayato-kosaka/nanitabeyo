...
## このスクリプトが答えないこと（#1815 水平展開の判定）

⚠️ 上の表の «パイプライン製 → 取り込み時の国» は **«値が入るか» の根拠であって
«値が正しいか» の根拠ではない。** 取り込みが `country='JP'` で絞った結果に
`country='JP'` が入っているのは当たり前で、それは «取り込み条件を検査条件に流用» した
形と同じである。実際 restaurant_catalog は全行 `country_code='JP'` のまま
**100,063 行（16.13%）が韓国・ロシアの店**だった。

このスクリプトは «NOT NULL にできるか（全行埋まるか）» の監査としては成立するので
残す。**«入っている国が合っているか» は `9_9_audit_foreign_rows.py` が
（取り込みから独立した根拠で）数える。** 2 つを混同しないこと。
