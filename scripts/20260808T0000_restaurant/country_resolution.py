...
#    書いていない行が多い**（`高知市追手筋1-3-11` / `卸町2-8-2` / `根津2-32-8`）。
#    厳しすぎて 98,190 行が «決められない» に落ちた（run 34032307384）。
#
# ハングル・キリルは先に判定して抜けているので、ここへ来る漢字混じりの住所は
# 日本・中国語圏のどちらかである。**日本にしか無い書き方**を並べる。
_JP_ADDRESS = re.compile(
    r"(?:[都道府県]\s*[^\s]*[市区町村郡])"  # 「長崎県対馬市」
    r"|(?:[^\s]{1,6}[市区町村郡][^\s])"  # 「高知市追手筋」「城南区樋井川」
    r"|(?:丁目|番地|[０-９0-9]\s*条[西東])"
    r"|(?:^\s*〒?\d{3}-?\d{4})"  # 郵便番号
    r"|(?:[一-鿿][^\s]*[０-９0-9]+[-−ー][０-９0-9]+)"  # 「根津2-32-8」「卸町2-8-2」
    r"|(?:\bChome\b)"  # ローマ字の「丁目」。韓国の住所には出ない
)
