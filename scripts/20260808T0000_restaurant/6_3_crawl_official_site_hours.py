...
公式サイトを持つ店のうち **11.3% は $0 の決定的パーサで構造化できる**。
その分をここで実際に入れる。LLM は使わない（要否は実測の残りを見てから決める）。

⚠️ **この 11.3% の分母は «公式サイトを持つ店» である**（到達できたページではない）。
2026-09-06 に 400 件・seed 16663・`--only-missing` なしで測り直して裏を取った
（[run 34041577304](https://github.com/Ayato-kosaka/nanitabeyo/actions/runs/34041577304)）:
候補 400 → 到達 242 → 日本語 212 → **parsed 45 = 候補の 11.3% / 日本語ページの 21.2%**。
**率を書くときは必ず分母を書くこと。** 分母を確かめずに 2 つの率を並べて
«一致した» と誤った報告を出したことがある（[訂正](https://github.com/Ayato-kosaka/nanitabeyo/issues/1666#issuecomment-5560166719)）。
    ...
    try:
        ...
        with connection.cursor() as cursor:
            for i, (rid, name, url) in enumerate(candidates, start=1):
                ...
                if (i - 1) % 25 == 0 and i > 1:
                    LOGGER.info("  … %d/%d 件（%s）", i - 1, len(candidates), dict(counts))
