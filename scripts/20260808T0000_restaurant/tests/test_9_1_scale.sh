...
#
# ## dev の実測（2026-09-23 / #1881）— この節が「確かめること」の答えである
#
# 国コードの是正で catalog の row_hash が **全行**変わり、`update=621966`（全件）になった
# 状態で測った。ここが «いちばん重い日» の実測値である。
#
#   ジョブ開始 → 同期計画（BigQuery export + COPY）      約 37 分
#   place id 付け替え / provenance 外し / 不足行 INSERT   バッチあたり 数秒〜1 分
#   値 UPDATE（50,000 行 / バッチ）                       **約 25 分**  ← 支配的
#
# ⚠️ **1 文で 62 万行を書くと 30 分の statement_timeout に必ず当たる**（run 35888471165）。
#    対策は «速くする» ではなく «1 文あたりの行数を有限にする»（execute_in_key_ranges）。
# ⚠️ **遅さの主因は走査ではなく書き込みである。** 先行する 3 群は同じ 62 万行の staging を
#    読んでいるのにバッチあたり数秒で終わる。値 UPDATE だけが 1,000 行あたり 30 秒かかる。
# ⚠️ したがって効く打ち手は «書く行を減らす» ことだった。ハッシュ差だけを条件にしていた
#    ため、**値が 1 つも変わっていない行まで全部書き直していた**（#2021 で是正）。
#
