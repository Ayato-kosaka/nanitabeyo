...
## 初回の実測（dev / 2026-09-05 06:23 UTC。run 33949678850）

次に走らせたときの比較対象として残す。**この数字を更新するときは実測で置き換えること。**

| 数えたもの | 値 |
| --- | --- |
| dish_media 総数 | 145,412（うち external_embed 140,510） |
| usable（4 条件通過） | 145,392（99.99%） |
| 可視（店舗詳細の 1 dish 1 件） | **38,957 = 総数の 26.79%** |
| 見えない行 | 106,435 |
| (店, カテゴリ) の異なり数 usable / 可視 | 38,957 / 38,957 — **差 0** |
| 店の異なり数 usable / 可視 | 21,507 / 21,507 — **差 0** |
| 真っ黒（3 段とも絵が無い） | usable 3,119（2.15%）／可視 1,080（2.77%） |
| 真っ黒のカテゴリ | 221 種類（**JP ゲートの 134 カテゴリには 0**） |
| 真っ黒を落として失う (店, カテゴリ) 組 | 1,080 組（うち JP ゲート **0 組**） |

**«見えない 106,435 行» は KPI に 1 セルも効いていない。** `dishes` は
(restaurant_id, category_id) で一意なので、可視集合は «(店, カテゴリ) につき 1 件» と
同義であり、KPI が数える «異なり店» はどちらの集合でも同じになる（差 0 が実測）。
効いているのは «同じ料理の 2 件目以降が読めない» という体験の側だけである。
