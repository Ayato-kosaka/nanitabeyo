...
class BigQueryLoader:
    ...
    def load_llm_labels(
        ...
        Args:
            labels: ラベル情報のリスト
                    #548: [{'item_qid': 'Q12345', 'label': 'keep', 'confidence': 'high', 'reason': '...'}, ...]
                    #550: [{'item_qid': 'Q12345', 'decision': 'A', 'confidence': 'high', 'macro_genre': 'ramen', 'reason': '...'}, ...]
            task: タスク識別子（例: '#548_menu_blacklist_classification' or '#550_macro_genre_abc_classification'）
        ...
        # データを準備（task により label/decision を判別）
        ...
        for label in labels:
            ...
            # #550 【設計】decision (A/B/C) または label (keep/too_generic...) を設定
            if "decision" in label:
                # #550 macro_genre task
                row["label"] = label["decision"]  # A/B/C を label フィールドに格納
                # macro_genre は note に格納（既存スキーマ流用）
                if label.get("macro_genre"):
                    row["note"] = label["macro_genre"]
            else:
    ...
    def get_macro_genre_label_targets(self) -> List[Dict]:
        """
        #550 【設計】macro_genre LLM 分類の対象ノードを取得
        dish_blacklist 未該当の全 dish（macro_genre 付与済みも含む）
        
        Returns:
            対象ノードのリスト [{'item_qid': 'Q12345', 'label_en': '...', 'desc_en': '...', 'tags': [...]}, ...]
        """
        logger.info("Fetching macro_genre label targets from BigQuery...")
        ...
        LEFT JOIN black
          ON dcc.item_qid = black.dish_qid
        WHERE black.dish_qid IS NULL
          AND dcc.label_en IS NOT NULL
        """
        ...
        logger.info(f"Found {len(nodes)} macro_genre label targets")
        return nodes
    
    def apply_macro_genre_llm_results_to_blacklist(self, task: str, run_id: str) -> int:
        """
        #550 【設計】decision=A (高信頼) を dish_blacklist に反映
        
        Args:
            task: タスク識別子（'#550_macro_genre_abc_classification'）
            run_id: 適用する run_id
            
        Returns:
            追加された行数
        """
        logger.info(f"Applying macro_genre LLM results (decision=A) from run_id={run_id} to dish_blacklist...")
        ...
        SELECT
          ...
          CONCAT('decision=', label, ', reason: ', reason) AS note,
          CURRENT_TIMESTAMP() AS created_at
        FROM `{self.dataset_ref}.wikidata_food_llm_labels`
        WHERE
          task = '{task}'
        ...
        affected = self.execute_dml(sql)
        logger.info(f"Applied {affected} decision=A items to dish_blacklist (run_id={run_id})")
        return affected
