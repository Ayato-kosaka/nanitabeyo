...
【目的】
Pass2 の結果を BigQuery wikidata_food_llm_labels テーブルにロード

【処理内容】
1. p2_results.jsonl を読み込み
...
python3 2_4_load_results.py --config config.yml
"""
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Load Pass2 results to BigQuery")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 2-4: Load Pass2 results to BigQuery")
    logger.info("=" * 80)
    ...
    if not results_file.exists():
        ...
        logger.error("Please run 2_3_poll_batch.py first")
        return
    ...
    # 一時 JSONL 作成
    temp_jsonl = results_dir / "p2_labels_for_bq.jsonl"
    ...
    metrics_collector.collect_pass2_metrics(parsed_items, batch_responses)
    ...
    logger.info("  python3 2_5_publish_features.py (after Pass2 is complete)")
