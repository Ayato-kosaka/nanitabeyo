...
【目的】
Pass1 バッチのステータスをポーリングし、完了後に結果をダウンロード

【処理内容】
1. p1_batch_id.txt からバッチIDを読み込む
2. バッチステータスをポーリング
3. 完了後に結果ファイルをダウンロード（p1_results.jsonl）
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Poll Pass1 batch status and download results")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 1-3 (poll): Poll Pass1 batch status and download results")
    logger.info("=" * 80)
    ...
    output_path = results_dir / "p1_results.jsonl"
    count = batch_client.download_results(output_file_id, output_path)
    
    logger.info("=" * 80)
    logger.info("✅ Step 1-3 (poll) completed successfully!")
    logger.info("=" * 80)
