...
【目的】
Pass1 input から Batch API 用のペイロードを生成

【処理内容】
1. p1_input.jsonl を読み込む
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Build Pass1 batch payload")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 1-2: Build Pass1 batch payload")
    logger.info("=" * 80)
    logger.info(f"Model: {config['model_pass1']}")
    logger.info(f"Batch size: {config['batch_size']}")
    
    # 入力ファイルパス
    input_dir = Path(config['input_export_path'])
    input_file = input_dir / "p1_input.jsonl"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for idx, batch in enumerate(batches):
            custom_id = f"p1_batch_{idx:05d}"
            request = create_batch_request(batch, custom_id, config['model_pass1'])
