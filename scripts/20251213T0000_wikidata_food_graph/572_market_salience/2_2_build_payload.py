...
# -*- coding: utf-8 -*-
"""
2_2_build_payload.py

【目的】
Pass2 input から Batch API 用のペイロードを生成

【処理内容】
1. p2_input.jsonl を読み込む
2. バッチサイズ（config.batch_size）ごとに分割
3. system prompt + user message + tool spec を含む Batch API ペイロードを生成
4. batch_payload_p1.jsonl に出力

【使用方法】
python3 2_2_build_payload.py
python3 2_2_build_payload.py --config config.yml --force
"""
...
# #572 【設計】lib/prompts モジュールをインポート
sys.path.insert(0, str(Path(__file__).parent))
from lib.io import load_yaml_config, read_jsonl, file_exists_or_skip, ensure_directory
from prompts.jp_market_salience import build_system_prompt, build_user_message, build_tool_spec
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Build Pass2 batch payload")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 2-2: Build Pass2 batch payload")
    logger.info("=" * 80)
    logger.info(f"Model: {config['model_pass2']}")
    logger.info(f"Batch size: {config['batch_size']}")
    
    # 入力ファイルパス
    input_dir = Path(config['input_export_path'])
    input_file = input_dir / "p2_input.jsonl"
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        for idx, batch in enumerate(batches):
            custom_id = f"p2_batch_{idx:05d}"
            request = create_batch_request(batch, custom_id, config['model_pass2'])
    ...
    logger.info("  2. Run: python3 2_3_submit_batch.py (or manually submit)")
