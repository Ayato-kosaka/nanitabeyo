...
【目的】
Pass1 バッチを OpenAI Batch API に投入し、batch_id を保存

【処理内容】
1. batch_payload_p1.jsonl を OpenAI にアップロード
2. バッチジョブを作成
3. batch_id を保存（p1_batch_id.txt）
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Submit Pass1 batch to OpenAI")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 1-3: Submit Pass1 batch to OpenAI Batch API")
    logger.info("=" * 80)
    ...
    if not payload_file.exists():
        ...
        logger.error("Please run 1_2_build_payload.py first")
        return
    ...
    logger.info(f"  2. Check status: python3 1_3_poll_batch.py")
