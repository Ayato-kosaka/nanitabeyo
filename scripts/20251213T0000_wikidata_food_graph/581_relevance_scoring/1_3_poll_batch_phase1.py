...
【目的】
Phase1 Batch API の完了をポーリング

【処理内容】
1. config.yml を読み込む
2. batch_id_phase1.txt から batch_id を読み込む
3. バッチが完了するまでポーリング
4. 完了後、結果をダウンロード

【使用方法】
python3 1_3_poll_batch_phase1.py
python3 1_3_poll_batch_phase1.py --config config.yml
"""
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Poll Phase1 Batch API completion")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 1-3: Poll Phase1 Batch API completion")
    logger.info("=" * 80)
    ...
    if not batch_id_file.exists():
        ...
        logger.error("Please run 1_3_submit_batch_phase1.py first")
        sys.exit(1)
    ...
    output_file = results_dir / "results_phase1.jsonl"
    ...
    logger.info("  python3 1_4_load_results_phase1.py")
