...
【目的】
Phase2 Batch API ペイロードを投入

【処理内容】
1. config.yml を読み込む
2. batch_payload_phase2.jsonl をアップロード
3. Batch API に投入
4. batch_id を保存

【使用方法】
python3 2_2_submit_batch_phase2.py
python3 2_2_submit_batch_phase2.py --config config.yml
"""
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Submit Phase2 Batch API payload")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 2-2: Submit Phase2 Batch API payload")
    logger.info("=" * 80)
    ...
    # ペイロードファイル確認
    payload_file = Path(config['payload_dir']) / "batch_payload_phase2.jsonl"
    if not payload_file.exists():
        logger.error(f"Payload file not found: {payload_file}")
        logger.error("Please run 2_1_build_payload_phase2.py first")
        sys.exit(1)
    ...
    logger.info("  python3 2_2_poll_batch_phase2.py")
    logger.info("")
