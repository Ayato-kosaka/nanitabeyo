...
【目的】
入力データを BigQuery からエクスポート

【処理内容】
1. config.yml を読み込む
2. export_input.sql を実行
...
def main():
    """メイン処理"""
    parser = argparse.ArgumentParser(description="Export input data from BigQuery")
    parser.add_argument(
        "--config",
        type=str,
        default="config.yml",
    ...
    logger.info("=" * 80)
    logger.info("Step 1-1: Export input data from BigQuery")
    logger.info("=" * 80)
    ...
    # SQL クエリ読み込み
    sql_file = Path(__file__).parent / "sql" / "export_input.sql"
    ...
    logger.info("  python3 1_2_build_payload_phase1.py")
