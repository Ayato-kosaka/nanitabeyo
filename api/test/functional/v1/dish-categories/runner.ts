...
class HttpClient {
   ...
   */
  async makeRequest(
    params: Partial<QueryDishCategoryRecommendationsDto>
    ...
    for (let attempt = 1; attempt <= this.config.maxRetries + 1; attempt++) {
      try {
        ...
        const backoffMs = Math.min(
          this.config.retryBackoffBaseMs * Math.pow(2, attempt - 1),
  ...
  private async executeRequest(
    ...
    try {
      ...
      return data as QueryDishCategoryRecommendationsResponse;
    } catch (error) {
      clearTimeout(timeoutId);
...
export class TestRunner {
  ...
  private logWriter: NodeJS.WritableStream;
  ...
  constructor(private config: TestConfig) {
    ...
    this.tokenBucket = new TokenBucket(
      ...
      config.requestsPerMinute / 60 // Convert to tokens per second
    );
  ...
  async run(): Promise<TestStatistics> {
    ...
    try {
      ...
      const promises = combinations.map((params, index) => 
        this.executeTest(params, index + 1, combinations.length)
      );
