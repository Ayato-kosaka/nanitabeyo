...
export interface TestConfig {
  ...
  strategy: 'cartesian' | 'random' | 'stratified';
  maxRequests: number;
...
/**
 * Sample parameter values for different sampling strategies
 */
 ...
 */
export const DEFAULT_PARAMETER_SAMPLES: ParameterSamples = {
  addresses: [
    '東京都渋谷区',
    '大阪府大阪市',
    '愛知県名古屋市',
...
export function generateParameterCombinations(
  ...
  maxRequests: number
): Partial<QueryDishCategoryRecommendationsDto>[] {
  switch (strategy) {
    case 'cartesian':
...
function generateCartesianCombinations(
  ...
  const combinations: Partial<QueryDishCategoryRecommendationsDto>[] = [];
        ...
        for (const scene of [...samples.scenes, undefined]) {
          for (const mood of [...samples.moods, undefined]) {
            for (const restrictions of [...samples.restrictions, undefined]) {
              const combo: Partial<QueryDishCategoryRecommendationsDto> = {
                address,
                languageTag,
                ...(timeSlot && { timeSlot }),
                ...(scene && { scene }),
 ...
 */
export function parseConfig(args: string[] = []): TestConfig {
    ...
    switch (arg) {
      case '--strategy':
        if (nextArg && ['cartesian', 'random', 'stratified'].includes(nextArg)) {
          config.strategy = nextArg as TestConfig['strategy'];
          i++;
        }
        break;
      case '--max-requests':
