...
export const DEFAULT_CONFIG: TestConfig = {
  ...
  strategy: (process.env.RECO_SAMPLING_STRATEGY as SamplingStrategy) || 'stratified',
  ...
  csvOutputPath: process.env.CSV_OUTPUT_PATH || './test-results/dish-categories-recommendations.csv',
  logOutputPath: process.env.LOG_OUTPUT_PATH || './test-results/dish-categories-recommendations.log',
 ...
 */
export function generateParameterCombinations(config: TestConfig): QueryDishCategoryRecommendationsDto[] {
 ...
 */
function generateCartesianCombinations(maxRequests: number): QueryDishCategoryRecommendationsDto[] {
          ...
          for (const mood of [undefined, ...TEST_PARAMETERS.moods]) {
            for (const restrictions of TEST_PARAMETERS.restrictions) {
              const params: QueryDishCategoryRecommendationsDto = {
                ...
                ...(scene && { scene }),
                ...(mood && { mood }),
                ...(restrictions.length > 0 && { restrictions: [...restrictions] }),
              };
 ...
 */
function generateRandomCombinations(maxRequests: number): QueryDishCategoryRecommendationsDto[] {
  const combinations: QueryDishCategoryRecommendationsDto[] = [];
 ...
 */
function generateStratifiedCombinations(maxRequests: number): QueryDishCategoryRecommendationsDto[] {
  const combinations: QueryDishCategoryRecommendationsDto[] = [];
  ...
  const samplesPerStratum = Math.ceil(maxRequests / (addressCount * languageCount));
  
  for (const address of TEST_PARAMETERS.addresses) {
    for (const languageTag of TEST_PARAMETERS.languageTags) {
      for (let i = 0; i < samplesPerStratum && combinations.length < maxRequests; i++) {
        const params: QueryDishCategoryRecommendationsDto = {
          address,
          languageTag,
        };
        ...
        if (variation & 1) {
          params.timeSlot = TEST_PARAMETERS.timeSlots[i % TEST_PARAMETERS.timeSlots.length];
        }
        if (variation & 2) {
          params.scene = TEST_PARAMETERS.scenes[i % TEST_PARAMETERS.scenes.length];
        }
        if (variation & 4) {
          params.mood = TEST_PARAMETERS.moods[i % TEST_PARAMETERS.moods.length];
        }
        if (variation & 8) {
          const restrictionsArray = TEST_PARAMETERS.restrictions[i % TEST_PARAMETERS.restrictions.length];
          if (restrictionsArray.length > 0) {
            params.restrictions = [...restrictionsArray];
          }
        }
