...
describe('#1798 usable-dish-media-filter の定義そのもの', () => {
  it('4条件（論理削除 / 退会 / 実体到達 / 再生可否）を全て含む', () => {
    ...
    expect(FILTER_SOURCE).toContain(
...
describe('#1798 findDishMediaIds と findDishMediaByRestaurant は同じ判定を参照する', () => {
  ...
  /*
  ⚠️ ここが #1798 の再発防止の要。呼び出し箇所は必ず 2 箇所（findDishMediaIds の
     base_candidates と findDishMediaByRestaurant の media_like_counts）。
     どちらかが条件を個別に書き戻す、あるいは参照を消すと 2 という数が崩れる。
  */
  it('${USABLE_DISH_MEDIA_CONDITIONS} の埋め込みが呼び出し側にちょうど2箇所ある', () => {
    const occurrences = REPOSITORY_SOURCE.split(
  ...
  it('findDishMediaByRestaurant 側にも埋め込まれている（店舗詳細で playback_status の除外が効く）', () => {
    ...
    expect(restaurantMethodSource).toContain(
      '${USABLE_DISH_MEDIA_CONDITIONS}',
    );
