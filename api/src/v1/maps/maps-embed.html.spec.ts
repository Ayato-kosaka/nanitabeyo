...
import { buildMapsEmbedSrc, escapeHtml, renderMapsEmbedPage } from './maps-embed.html';
...
describe('buildMapsEmbedSrc', () => {
  it('search モード: mode/q/key を含む Maps Embed API の URL を組み立てる', () => {
    const src = buildMapsEmbedSrc({ mode: 'search', q: 'ラーメン 渋谷', apiKey: 'test-key' });
  ...
  it('place モード: q に place_id:<id> をそのまま渡す', () => {
    const src = buildMapsEmbedSrc({ mode: 'place', q: 'place_id:ChIJplace1', apiKey: 'test-key' });
    const url = new URL(src);
  ...
  it('center / zoom / hl は指定したときだけ含める', () => {
    const withoutOptional = new URL(buildMapsEmbedSrc({ mode: 'search', q: 'ramen', apiKey: 'k' }));
    ...
    const withOptional = new URL(
      buildMapsEmbedSrc({ mode: 'search', q: 'ramen', center: '35.6,139.7', zoom: 15, hl: 'ja', apiKey: 'k' }),
    );
...
describe('renderMapsEmbedPage', () => {
  it('iframe の src 属性へ埋め込む。key は本文全体でちょうど 1 回しか現れない', () => {
    const src = buildMapsEmbedSrc({ mode: 'search', q: 'ramen', apiKey: 'super-secret-key' });
  ...
  it('q に混ぜたスクリプトタグは本文へそのまま出ない（エスケープされる）', () => {
    const src = buildMapsEmbedSrc({ mode: 'search', q: `"><script>alert(1)</script>`, apiKey: 'k' });
    const html = renderMapsEmbedPage(src);
