...
import { signMapsEmbedToken, verifyMapsEmbedToken, MAPS_EMBED_TOKEN_TTL_MS } from './maps-embed.token';
...
describe('MapsController（キー設定済み）', () => {
  ...
  describe('AuthAnonGuard が付いていること（メタデータ）', () => {
    ...
    it('POST /v1/maps/embed-token には AuthAnonGuard が付いている', () => {
      const guards = Reflect.getMetadata(GUARDS_METADATA, MapsController.prototype.createEmbedToken);
      expect(guards).toContain(AuthAnonGuard);
    });

    it('GET /v1/maps/embed にはガードを付けない（WebView が Authorization を送れないため設計どおり）', () => {
      const guards = Reflect.getMetadata(GUARDS_METADATA, MapsController.prototype.getEmbed);
  ...
  describe('POST /v1/maps/embed-token', () => {
    ...
    it('正常系: token と expiresAt を返し、token は自分自身の署名鍵で検証できる', async () => {
      ...
      const res = await request(app.getHttpServer())
        .post('/v1/maps/embed-token')
        .set('Authorization', validAuthHeader())
        .send({ mode: 'search', q: 'ラーメン 渋谷', center: '35.6,139.7', zoom: 15, hl: 'ja' })
        .expect(201);

      expect(typeof res.body.token).toBe('string');
      expect(typeof res.body.expiresAt).toBe('string');

      const verified = verifyMapsEmbedToken(res.body.token, SUPABASE_JWT_SECRET, Date.now());
      ...
      expect(expiresAt).toBeGreaterThanOrEqual(before + MAPS_EMBED_TOKEN_TTL_MS);
      expect(expiresAt).toBeLessThanOrEqual(Date.now() + MAPS_EMBED_TOKEN_TTL_MS);
    });
  });
  ...
  describe('GET /v1/maps/embed', () => {
    ...
    it('署名が壊れた token は 401', async () => {
      ...
      await request(app.getHttpServer()).get('/v1/maps/embed').query({ token: tampered }).expect(401);
    });
    ...
    it('期限切れの token は 401', async () => {
      ...
      await request(app.getHttpServer()).get('/v1/maps/embed').query({ token: expiredToken }).expect(401);
    });

    it('正常系: 200 / text/html で iframe を含む HTML を返す', async () => {
      const token = signMapsEmbedToken(
        { mode: 'search', q: 'ラーメン 渋谷', center: '35.6,139.7', zoom: 15, hl: 'ja' },
        SUPABASE_JWT_SECRET,
        Date.now(),
      );

      const res = await request(app.getHttpServer()).get('/v1/maps/embed').query({ token }).expect(200);
    ...
    it('place モード: q=place_id:<id> をそのまま Embed API へ渡す', async () => {
      const token = signMapsEmbedToken({ mode: 'place', q: 'place_id:ChIJplace1' }, SUPABASE_JWT_SECRET, Date.now());
