...
export class FeedbackService {
  ...
  async createIssue(feedbackDto: CreateFeedbackDto): Promise<CreateFeedbackResponseDto> {
...
*This issue was automatically created from the mobile app feedback system.*
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(`GitHub API error: ${response.status} ${response.statusText} - ${errorText}`);
        throw new Error(`Failed to create GitHub issue: ${response.statusText}`);
      }

      const issue: GitHubIssue = await response.json();

      this.logger.log(`Created GitHub issue #${issue.number} for ${type} feedback: ${title}`);
