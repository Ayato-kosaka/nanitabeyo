...
import { buildOpenInAppUrl, buildTargetWebPath, SHARE_PAGE_LABELS } from './share-links.web-path';
...
describe('#721 共有 target → Web パス', () => {
  describe('dish_media', () => {
    it('既存の /{locale}/posts?ids=... をそのまま使う', () => {
      ...
      const path = buildTargetWebPath('dish_media', 'id-1', { ids: ['id-1', 'id-2'] }, 'ja-JP');
    ...
    it('ids の中身を encode する', () => {
      ...
      // クエリを抜けられないことを固定する
      const path = buildTargetWebPath('dish_media', 'x', { ids: ['a&b=c', '<script>'] }, 'ja-JP');
  ...
  describe('dish_category_group_vote_sessions', () => {
    ...
    it('shareToken が失われている行はホームへ落とす', () => {
      ...
      const path = buildTargetWebPath('dish_category_group_vote_sessions', 'session-uuid', {}, 'ko-KR');
  ...
  it('返すのは常に相対パスで、ホストを含まない', () => {
    ...
    for (const locale of PUBLIC_LOCALES) {
      const path = buildTargetWebPath('dish_media', 'id', { ids: ['id'] }, locale);
  ...
  describe('アプリで開く導線', () => {
    it('OIA relay 経由にする（iOS の同一サイト起点 UL 抑止を避けるため）', () => {
      const url = buildOpenInAppUrl('https://app.nanitabeyo.net/ja-JP/posts?ids=a');

      expect(url.startsWith('https://oia-relay.web.app/oia/open/?u=')).toBe(true);
      // relay 側が origin を検証できるよう、URL 全体を encode して渡す
      expect(url).toContain(encodeURIComponent('https://app.nanitabeyo.net/ja-JP/posts?ids=a'));
    });
  });
