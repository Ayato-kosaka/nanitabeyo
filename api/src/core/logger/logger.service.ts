...
export class AppLoggerService implements INestLoggerService {
    ...
    verbose(eventName, functionName, payload: any) {
        ...
        this.persistBackendEvent({ error_level: LogLevel.verbose, payload, event_name: eventName, function_name: functionName });
    }
    debug(eventName, functionName, payload: any) {
        if (this.shouldPrint(LogLevel.debug)) console.debug(payload);
        this.persistBackendEvent({ error_level: LogLevel.debug, payload, event_name: eventName, function_name: functionName });
    }
    log(eventName, functionName, payload: any) {
        if (this.shouldPrint(LogLevel.log)) console.info(payload);
        this.persistBackendEvent({ error_level: LogLevel.log, payload, event_name: eventName, function_name: functionName });
    }
    warn(eventName, functionName, payload: any) {
        if (this.shouldPrint(LogLevel.warn)) console.warn(payload);
        this.persistBackendEvent({ error_level: LogLevel.warn, payload, event_name: eventName, function_name: functionName });
    }
    error(eventName, functionName, payload: any) {
        if (this.shouldPrint(LogLevel.error)) console.error(payload);
        this.persistBackendEvent({ error_level: LogLevel.error, payload, event_name: eventName, function_name: functionName });
    }

    /* ------------------------------------------------------------------ */
    /*            外部 API コールを詳細に残すための専用メソッド           */
    /* ------------------------------------------------------------------ */
    async externalApi(input: Required<Omit<Prisma.external_api_logsCreateInput, "id" | "user_id" | "request_id" | "created_commit_id" | "created_at">>) {
        try {
            ...
            console.error('Failed to persist external_api_logs:', (err as Error).message);
        }
    }
    ...
    /*                           private helpers                          */
    /* ------------------------------------------------------------------ */
    private async persistBackendEvent(input: Required<Omit<Prisma.backend_event_logsCreateInput, "id" | "user_id" | "request_id" | "created_commit_id" | "created_at">>) {
        try {
            ...
            console.error('Failed to persist backend_event_logs:', (err as Error).message);
        }
    }
    ...
    private shouldPrint(level: LogLevel): boolean {
        ...
        return (
            order.indexOf(level) >= order.indexOf(this.minLevel)
        );
    }
}
