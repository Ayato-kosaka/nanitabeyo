...
describe('Issue #291: Japanese terminal search showing English topics', () => {
  describe('before fix (static schema)', () => {
    it('would allow English text in Japanese language context', () => {
          ...
          items: {
            items: {
              properties: {
                topicTitle: {
                  type: 'string',
                  description: 'Catchy topic title',
                },
      ...
      const problematicTopicTitle =
        'Georgian Cuisine: A Flavorful Journey Through Kutaisi';
      const problematicReason =
        'Immerse yourself in the rich culinary traditions of Kutaisi with a selection of authentic Georgian dishes';
  ...
  describe('after fix (language-specific schema)', () => {
    it('should prevent English text when languageTag is ja-JP', () => {
      ...
      const topicTitlePattern = (
        schema.input_schema.properties.items.items.properties.topicTitle as any
      ).pattern;
      const reasonPattern = (
        schema.input_schema.properties.items.items.properties.reason as any
      ).pattern;
      ...
      expect(
        reasonRegex.test(
          'Immerse yourself in the rich culinary traditions of Kutaisi',
        ),
      ).toBe(false);
      ...
      expect(
        reasonRegex.test('今日のランチにぴったりな料理をご紹介します'),
      ).toBe(true);
    });
    ...
    it('should never constrain category field (always English Wikidata)', () => {
      ['ja-JP', 'en-US', 'zh-CN'].forEach((languageTag) => {
        ...
        const categoryProps = schema.input_schema.properties.items.items
  ...
  describe('acceptance criteria validation', () => {
    it('should meet all acceptance criteria from the issue', () => {
      ...
      const jaTopicPattern = (
        jaSchema.input_schema.properties.items.items.properties
          .topicTitle as any
      ).pattern;
      const jaReasonPattern = (
        jaSchema.input_schema.properties.items.items.properties.reason as any
      ).pattern;
      ...
      const enTopicPattern = (
        enSchema.input_schema.properties.items.items.properties
          .topicTitle as any
      ).pattern;
      const enReasonPattern = (
        enSchema.input_schema.properties.items.items.properties.reason as any
      ).pattern;
      ...
      const zhTopicPattern = (
        zhSchema.input_schema.properties.items.items.properties
          .topicTitle as any
      ).pattern;
      const zhReasonPattern = (
        zhSchema.input_schema.properties.items.items.properties.reason as any
      ).pattern;
