 ...
 * 余白ゼロ(=中央カラム幅そのまま)の値を使うよう変更した(app-expo/features/topics/hooks/useTopicCardSize.ts)。
 *
...
test.describe("候補カルーセルのカード幅とスナップ位置(#1212)", () => {
	// トピック生成に時間がかかる(実測30秒近くかかることがある)ため、topics-flow.spec.ts と同じ理由で延長する。
	test.setTimeout(90_000);

	async function gotoTopics(searchPage: SearchPage, topicsPage: TopicsPage): Promise<void> {
	...
	test("カード幅は中央カラム幅と一致し、左右の余白が無い", async ({ appPage }) => {
		...
		await gotoTopics(searchPage, topicsPage);

		const activeSwipeArea = appPage.getByTestId("topics-tutorial-target-swipe");
	...
	test("スワイプ後もカードは画面中央のスナップ位置で止まる(横位置がズレない)", async ({ appPage }) => {
		...
		const mountedCardCount = await appPage.getByTestId("topics-choose-button").count();
