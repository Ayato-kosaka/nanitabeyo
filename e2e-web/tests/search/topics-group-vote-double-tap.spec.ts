...
test.describe("友達投票開始ボタンの連打耐性 @mutation", () => {
	...
	// topics-flow.spec.ts と同じ理由で既定の 30 秒テストタイムアウトを延長する
	...
	//        ガードの解除は topics.tsx の useFocusEffect が握っており、
	...
	test("友達投票開始ボタンを連打しても投票セッションは 1 件しか作られない", async ({ appPage }) => {
		...
		await clickRapid(topicsPage.groupVoteButton, PRESS_COUNT);

		// 結果画面の描画（= 作成 API の完了 + detail 取得）まで待ってから数える。
		// 二重 push はこの時点までに必ず DOM へ載っているため、1 枚目だけを見て取りこぼすことはない。
		// ⚠️ `.first()` ではなく `.last()` で待つこと（TopicsPage.expectRenderedAllowingDuplicates と同じ理由）
