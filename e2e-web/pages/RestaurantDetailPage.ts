 ...
 *
 * ⚠️ 共通の `screen-header-back` を使わないこと（#1394）。ルート化で push した画面と背面の画面が
...
export class RestaurantDetailPage {
	...
	/** 店舗詳細のヘッダー戻る。#1394 で画面ごとの id になった */
	...
	async expectBidOpened(): Promise<void> {
		await expect(this.page).toHaveURL(/\/review\/restaurant\/[^/]+\/bid(\?.*)?$/);
	 ...
	 * DOM に残るので、押せてしまうが «押しているのは背面の画面» になる（#1394）。
	 */
