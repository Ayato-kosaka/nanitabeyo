...
- 保存した料理カテゴリの «地点検索» を画面として提供する（中身は
  features/profile/containers/SavedTopicLocationSearch）。
- 検索対象のカテゴリを URL パラメータ（topicId / topicLabelEn）で受け取る。
...
パスをマイページ配下（`profile/saved-topic-location`）に置いたのは、遷移先の検索結果
...
export default function SavedTopicLocationScreen() {
	...
	const { topicId, topicLabelEn } = useLocalSearchParams<{ topicId?: string; topicLabelEn?: string }>();
		...
		<LinearGradient colors={colors.backgroundGradient} style={styles.container}>
			<SafeAreaView style={styles.safeArea} edges={[]}>
				<ScreenHeader
					...
					testID="saved-topic-location-screen"
				/>
				<SavedTopicLocationSearch topicId={topicId} topicLabelEn={topicLabelEn} />
			</SafeAreaView>
		</LinearGradient>
	);
}
