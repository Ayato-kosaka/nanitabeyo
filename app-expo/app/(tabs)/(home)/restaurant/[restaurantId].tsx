	...
	const renderStars = (rating: number) => {
		...
		return (
			<View style={styles.starsContainer}>
				{[...Array(5)].map((_, index) => (
					<Star key={index} size={16} color="#FFD700" fill={index < fullStars ? "#FFD700" : "transparent"} />
				))}
			</View>
		);
	};
						...
						<View style={styles.filterSection}>
							...
							<View style={styles.filterOptions}>
								{priceRanges.map((range) => (
									<TouchableOpacity
										key={range}
										style={[styles.filterOption, filters.priceRange === range && styles.filterOptionSelected]}
										onPress={() =>
											setFilters((prev) => ({
												...prev,
