...
export const FixedColors = {
	...
	*/
	...
	/*
	#1834【チーム指摘】«食べたい» 側の色相。**塗りの有無に «色相» を足す。**

	「食べたい、食べたはオレンジ、オレンジ囲みで色分けされてるが、食べたいのボタンは
	 緑色にするとか、色を変えたほうが視覚的に見分けられやすいと思った」。

	⚠️ **塗りの有無をやめて色相だけにしない。** 塗りの差は色覚に依存せず、
	   サムネイルの上でも «埋まっているか» が形として読める（`statusColors.ts` の JSDoc）。
	   足すのであって、置き換えるのではない。
	⚠️ **これより明るい緑へ動かさないこと。** `#2E7D32` は白との対比 5.13:1 で、
	   白地に載る小さな太字（«食べたい» バッジの文字）の下限 4.5:1 を満たす
	   （相方のオレンジ `#ED6C02` は 3.11:1 しかないので、こちら側の方が読みやすい）。
	*/
	myDishStatusGreen: "#2E7D32",
	/** 上の 2 つと 1 組。白塗り側の地であり、オレンジ塗り側の字・縁でもある */
