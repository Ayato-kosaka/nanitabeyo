...
/**
 * 🧪 E2E(Detox) 専用: 検索チュートリアルの「視聴済みフラグ」を起動引数で固定する。
 *
 * ## なぜ必要か（#1027 / #1031 B4）
 * 検索チュートリアル（TutorialBottomSheet）は ja-JP かつ未視聴のとき **初回起動で自動的に開く**。
 * 実体は TrueSheet（Android では別ウィンドウの Dialog）で、開いている間は背後のタブバー・検索フォームへの
 * タップがシートに吸われる。つまり **チュートリアル関連以外の全 spec にとってはノイズ**でしかない。
 *
 ...
 *   シートが開いていてもタブバーは `toBeVisible()` を満たしてしまう（= 起動完了と誤判定する）
 ...
 * 永続化の検証がフックによって骨抜きにならない（e2e-mobile/tests/search/search-tutorial.test.ts）。
 *
 * ## 本番混入防止（#1030 と同一方式）
 ...
 * @失敗時 例外は投げない。ここで落とすと「チュートリアルの見え方」だけの都合でアプリが起動しなくなるため、
 *         読めなかった場合は「固定なし」として通常経路へ倒す（セッション注入フックの fail-loud とは方針が異なる。
 *         あちらは *誰でテストしているか* が嘘になるが、こちらは高々シートが 1 枚余分に出るだけ）
 */
