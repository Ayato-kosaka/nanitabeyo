...
import { fetchWithAuth } from "@/hooks/useAPICall";
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
	...
	const createUserProfile = useCallback(
			...
			try {
				...
				if (!existingProfileId) {
					...
					if (avatar) {
						...
						if (!token) throw new Error("User is not authenticated: Supabase access_token is missing.");
						await fetchWithAuth<UpdateUserProfileDto>(
							"v1/users/me",
							{
								method: "POST",
								requestPayload: {
		...
		[locale, logFrontendEvent, uploadFile, getSession],
	);
