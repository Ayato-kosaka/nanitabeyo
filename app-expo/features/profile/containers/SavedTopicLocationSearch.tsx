...
interface SavedTopicLocationSearchProps {
 ...
 *
 * #1369 【設計】以前は `SavedTopicsTab` の中の BlurModal だった。地点オートコンプリートを
 ...
 * #1133 で `SavedTopicsTab` が持っていたものをそのまま移設したもので、検証は
 * `SavedTopicLocationSearch.test.tsx` にある。ここを触ったら必ずテストを一緒に見ること。
 */
export function SavedTopicLocationSearch({ topicId, topicLabelEn }: SavedTopicLocationSearchProps) {
	...
	const handleLocationSelect = useCallback(
		async (selected: SelectedLocation) => {
			...
			if (!topicId || !topicLabelEn) return;
			...
			// 静かな退行になる（モーダル時代は SavedTopicsTab が残り続けていたので起きなかった）。
	...
	return (
		...
		<LocationSearchForm onSubmit={handleLocationSelect} title={null} testID="saved-topic-location-search" />
	);
}
