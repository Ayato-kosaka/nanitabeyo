 ...
 *
 * #1369 でモーダルからルートへ移したのに伴い、検証対象も `SavedTopicsTab` から
 * 遷移先の中身（`SavedTopicLocationSearch`）へ移した（ファイル名も合わせて変えている）。
...
import { SavedTopicLocationSearch } from "./SavedTopicLocationSearch";

/** ルートのパラメータで運ばれてくる検索対象（保存料理カテゴリ） */
const TOPIC_ID = "cat-1";
const TOPIC_LABEL_EN = "ramen";
...
describe("#1133 保存料理カテゴリの地点確定（#1369 でルート化）", () => {
	...
	const selectLocation = async (selected: SelectedLocation) => {
		await act(async () => {
			renderer = TestRenderer.create(<SavedTopicLocationSearch topicId={TOPIC_ID} topicLabelEn={TOPIC_LABEL_EN} />);
		});
		await act(async () => {
	...
	it("トピックのパラメータが無ければ検索も遷移もしない", async () => {
		await act(async () => {
			renderer = TestRenderer.create(<SavedTopicLocationSearch />);
		});
		await act(async () => {
