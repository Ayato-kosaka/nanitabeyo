// TopicList - renders topics carousel with loading, error, and modal interactions
...
import { X, ThumbsUp, Sparkles } from 'lucide-react-native';
import { PrimaryButton } from '@/components/PrimaryButton';
import { Topic } from '@/types/search';
import { TopicCard } from '@/features/topics/TopicCard';
import { TopicsData } from '@/features/topics/hooks/useTopicsData';

const { width, height } = Dimensions.get('window');
const CARD_WIDTH = width - 32;
const CARD_HEIGHT = height * 0.85;

export function TopicList(props: TopicsData) {
  const {
    visibleTopics,
    currentIndex,
    setCurrentIndex,
  ...
  const renderCard = ({ item }: { item: Topic }) => (
    <TopicCard topic={item} onHide={handleHideCard} />
  );
      ...
      {visibleTopics.length > 0 ? (
        <View style={styles.carouselContainer}>
          <Carousel
            ...
            mode="parallax"
            modeConfig={{ parallaxScrollingScale: 0.9, parallaxScrollingOffset: 100 }}
