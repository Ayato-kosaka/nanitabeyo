...
//
// useTopicImageResources の web 分岐は、Blob を作らないために ready へ即座に進み、
// 実体の取得は表示側の <img> に任せる設計だった（#719/#929）。
// Topics 検索画面はカルーセルとサムネイルで全件を同時に描くので、それでも «全件が取得済み» に
...
import { useTopicImageResources } from "./useTopicImageResources";
...
const TOPICS: Topic[] = [0, 1, 2].map((index) => ({
	category: `cat-${index}`,
	categoryId: `cat-${index}`,
	topicTitle: `title-${index}`,
	reason: `reason-${index}`,
	imageUrl: `https://images.example.invalid/candidate-${index}.svg`,
}));

function Probe({ topics }: { topics: Topic[] }) {
	useTopicImageResources({ topics, sessionKey: "session-1213" });
	return null;
}

describe("useTopicImageResources（web）", () => {
	...
	it("マウント時に候補全件ぶんのブラウザ取得を開始する", () => {
		act(() => {
			TestRenderer.create(<Probe topics={TOPICS} />);
		});

		expect(FakeBrowserImage.requested).toEqual(TOPICS.map((topic) => topic.imageUrl));
	});
	...
	// 手順:
	//   1. マウント後、同じ topics で再描画させる
	//   2. 取得要求が増えていないことを検証する
	// 補足: loadTopicImage は loading / ready の候補を弾く。ここが壊れると
	//       再描画のたびにリクエストが積み上がる
	it("再描画しても同じ候補を取り直さない", () => {
		let renderer!: TestRenderer.ReactTestRenderer;
		act(() => {
			renderer = TestRenderer.create(<Probe topics={TOPICS} />);
		});
		const afterMount = [...FakeBrowserImage.requested];

		act(() => {
			renderer.update(<Probe topics={TOPICS} />);
		});
