 ...
 *
 * useTopicCardSize({ fullBleed: true }) は topics.tsx の Carousel だけが使う値で、
...
function Harness({
	...
	const result = useTopicCardSize(fullBleed === undefined ? undefined : { fullBleed });
	onResult(result);
	return null;
}

function renderTopicCardSize(windowWidth: number, fullBleed?: boolean) {
	...
	if (!captured) throw new Error("useTopicCardSize did not return a result");
	return captured;
}

describe("useTopicCardSize", () => {
	it("既定(fullBleed未指定)では左右16pxずつ(計32px)の余白を残す", () => {
		const { cardWidth, cardMaxHeight } = renderTopicCardSize(375);
	...
	it("fullBleed:true では画面幅と完全に一致する(余白ゼロ)", () => {
		const { cardWidth, cardMaxHeight } = renderTopicCardSize(375, true);
