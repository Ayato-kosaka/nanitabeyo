 ...
 * 一意制約で失敗し、catch の `setTopicSaved(item.categoryId, !willSave)` が発火して
 ...
 *
 * `useTopicsStore` は**本物を使う**。巻き戻りは「楽観更新 → catch でのロールバック」という
...
// entriesKey の定義だけを使うが、SavedTopicsTab 本体は画面まるごとを引き込むのでスタブ化する
jest.mock("@/features/profile/tabs/SavedTopicsTab", () => ({ profileSavedTopicsEntriesKey: "profileSavedTopics" }));
...
jest.mock("./TopicVisualCard", () => {
	const ReactModule = require("react");
	const { View: RNView } = require("react-native");
	return {
		TopicVisualCard: ({ topRightContent, bottomContent }: { topRightContent?: unknown; bottomContent?: unknown }) =>
...
describe("#1205 TopicCard の保存の多重実行ガード", () => {
	let tree: TestRenderer.ReactTestRenderer;

	beforeEach(async () => {
		// 保存状態は store が持つ。テスト間で持ち越すと fallback（item.isSaved）が効かなくなる
		useTopicsStore.setState({ savedByTopicId: {}, topicIdsByKey: {}, topicById: {} });
	...
	const saveButton = (): ReactTestInstance => {
		const button = tree.root.findAll(
			(node) =>
				...
				(node.props.accessibilityLabel === "Topics.accessibility.saveTopic" ||
					node.props.accessibilityLabel === "Topics.accessibility.unsaveTopic"),
		)[0];
	...
	it("二重に押しても表示が保存解除へ巻き戻らない（2 発目が一意制約で失敗しても）", async () => {
		...
		// #1205 これが本丸。ガードが無いと 2 発目の catch で setTopicSaved(false) が走り、
		// 保存できているのにブックマークが未保存表示へ戻る
		expect(isSavedOnScreen()).toBe(true);
		expect(useTopicsStore.getState().savedByTopicId[topic.categoryId]).toBe(true);
