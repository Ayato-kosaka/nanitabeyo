 ...
 * `profile/settings.tsx` に閉じていたロジックをそのままここへ出しただけで、
 * ダイアログの文言・ストア URL の選び方・フォールバックの順序は変えていない。
 *
 * ⚠️ web には «ストア» が無いので、呼び出し側で `Platform.OS !== "web"` を
 * 見て行ごと出さないこと（このフック自体はプラットフォームを判定しない）。
 */
