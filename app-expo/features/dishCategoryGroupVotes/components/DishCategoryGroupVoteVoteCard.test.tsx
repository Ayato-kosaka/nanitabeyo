...
// DishCategoryGroupVoteVoteCard は候補画像の描画を TopicVisualCard に委譲しているが、
// TopicVisualCard は「画面側が Image.loadAsync で先読みした ImageRef（imageState）」を
// 受け取って初めて "cachePolicy=memory 前提の即時表示" ができる設計になっている
// （features/topics/components/TopicVisualCard.tsx の #785/#802/#929 コメント参照）。
// 修正前の DishCategoryGroupVoteVoteCard は imageState を一切受け取らず、常に生の
// uri をその場で TopicVisualCard へ渡していたため、候補が変わるたびに
...
describe("DishCategoryGroupVoteVoteCard の画像プリロード連携", () => {
	...
	it("imageState が loading のあいだは、未ロードの生 uri で Image を描画しない", () => {
		...
		// TopicVisualCard は imageState が ready 以外のときは Image 自体を描画しない
