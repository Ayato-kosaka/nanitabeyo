...
import type {
	DishCategoriesTutorialOpenReason,
	DishCategoriesTutorialRect,
	DishCategoriesTutorialStepDefinition,
	DishCategoriesTutorialTargetKey,
	DishCategoriesTutorialTargetRefs,
} from "@/features/dishCategories/types/tutorial";
...
const TUTORIAL_STEPS: readonly DishCategoriesTutorialStepDefinition[] = [
	...
	},
	{
		...
		bodyKeys: ["DishCategories.tutorial.steps.dishCategoryActions.save", "DishCategories.tutorial.steps.dishCategoryActions.block"],
...
type DishCategoriesSpotlightTutorialProps = {
	visible: boolean;
	requestId: number;
	openReason: DishCategoriesTutorialOpenReason;
...
type MeasuredTargets = Partial<Record<DishCategoriesTutorialTargetKey, DishCategoriesTutorialRect>>;

const wait = (milliseconds: number) => new Promise<void>((resolve) => setTimeout(resolve, milliseconds));

/** 幅・高さが0のレイアウト値は「まだ描画されていない」として扱う。 */
const isUsableRect = (rect: DishCategoriesTutorialRect) =>
 ...
 */
const measureTarget = (
	targetKey: DishCategoriesTutorialTargetKey,
	targetRefs: DishCategoriesTutorialTargetRefs,
): Promise<DishCategoriesTutorialRect | null> => {
	const target = targetRefs[targetKey].current;
	if (!target) return Promise.resolve(null);

	return new Promise((resolve) => {
		let isSettled = false;
		const finish = (rect: DishCategoriesTutorialRect | null) => {
			if (isSettled) return;
			isSettled = true;
 ...
 */
const isSameRect = (a: DishCategoriesTutorialRect | null | undefined, b: DishCategoriesTutorialRect | null | undefined) => {
	if (!a || !b) return false;
	return (
		Math.abs(a.x - b.x) <= 1 &&
		Math.abs(a.y - b.y) <= 1 &&
...
const getUnionRect = (rects: readonly DishCategoriesTutorialRect[]): DishCategoriesTutorialRect | null => {
 ...
 */
const padRect = (rect: DishCategoriesTutorialRect, windowWidth: number, windowHeight: number): DishCategoriesTutorialRect => {
...
const getCalloutPosition = ({
	...
	targetRect: DishCategoriesTutorialRect;
	calloutWidth: number;
	...
	safeTop: number;
	safeBottom: number;
	preferredPlacement: DishCategoriesTutorialStepDefinition["preferredPlacement"];
}) => {
...
export function DishCategoriesSpotlightTutorial({
	...
	onClose,
	onUnavailable,
}: DishCategoriesSpotlightTutorialProps) {
	...
	const [availableSteps, setAvailableSteps] = useState<readonly DishCategoriesTutorialStepDefinition[]>([]);
	...
	const reportMissingTarget = useCallback(
		(step: DishCategoriesTutorialStepDefinition, targetKey: DishCategoriesTutorialTargetKey) => {
	...
	useEffect(() => {
		...
		const remeasureCurrentStep = async () => {
			let entries: readonly (readonly [DishCategoriesTutorialTargetKey, DishCategoriesTutorialRect | null])[] = [];
			let previousEntries: ReadonlyMap<DishCategoriesTutorialTargetKey, DishCategoriesTutorialRect | null> = new Map();
	...
	const activeTargetRects = useMemo(() => {
		...
		return currentStep.targetKeys
			...
			.filter((rect): rect is DishCategoriesTutorialRect => rect !== undefined)
	...
	useEffect(() => {
		...
		const progress = i18n.t("DishCategories.tutorial.progressAccessibility", {
	...
	const progressLabel = i18n.t("DishCategories.tutorial.progress", {
		current: currentStepIndex + 1,
		total: availableSteps.length,
	});
	const progressAccessibilityLabel = i18n.t("DishCategories.tutorial.progressAccessibility", {
		current: currentStepIndex + 1,
		total: availableSteps.length,
	});
	const maskId = `dish-categories-tutorial-mask-${requestId}`;
	...
	return (
		<Modal
			...
			<View style={styles.overlay} testID="dish-categories-tutorial-overlay" accessibilityViewIsModal pointerEvents="auto">
				...
				{showSwipeHint && swipeAreaRect && (
					<Animated.View
						pointerEvents="none"
						testID="dish-categories-tutorial-swipe-hint"
						style={[
							styles.swipeHint,
							{
				...
				<View
					...
					testID={`dish-categories-tutorial-step-${currentStep.id}`}>
					<View
						pointerEvents="none"
						style={[
							styles.arrow,
					...
					<ScrollView
						...
						<View style={styles.progressRow}>
							...
							<Text
								...
								testID="dish-categories-tutorial-progress">
								{progressLabel}
							</Text>
						</View>
						...
						<View style={styles.actions}>
							...
							{!isLastStep && (
								<TouchableOpacity
									...
									testID="dish-categories-tutorial-skip">
									<Text style={styles.skipButtonText}>{i18n.t("DishCategories.tutorial.skip")}</Text>
								</TouchableOpacity>
							)}
							<TouchableOpacity
								...
								accessibilityRole="button"
								testID={isLastStep ? "dish-categories-tutorial-finish" : "dish-categories-tutorial-next"}>
								<Text style={styles.primaryButtonText}>
									{i18n.t(isLastStep ? "DishCategories.tutorial.finish" : "DishCategories.tutorial.next")}
								</Text>
							</TouchableOpacity>
						</View>
					</ScrollView>
