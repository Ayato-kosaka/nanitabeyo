...
import type { DishCategoriesTutorialOpenReason } from "@/features/dishCategories/types/tutorial";
...
type TutorialRequest = {
	id: number;
	reason: DishCategoriesTutorialOpenReason;
};

type UseDishCategoriesTutorialOptions = {
	/**
	 * 料理取得、Carouselの高さ計算、検索セッション確定まで完了したか。
	 *
	 * ここではrefの座標までは判定しない。実座標の最終確認は
	 * DishCategoriesSpotlightTutorial が行い、表示成功後に onPresented を返す。
	 */
	canAutoOpen: boolean;
};
 ...
 */
export function useDishCategoriesTutorial({ canAutoOpen }: UseDishCategoriesTutorialOptions) {
	...
	useEffect(() => {
		...
		const loadTutorialState = async () => {
			...
			// canAutoOpen が成立する瞬間がずれ、iOS の search-double-tap / dish-categories-flow が
			// 「チュートリアルがヘッダーを覆って dish-categories-header-back を押せない」で落ちるようになった。
			//
			...
			try {
				const storedValue = await AsyncStorage.getItem(DISH_CATEGORIES_TUTORIAL_STORAGE_KEY);
				if (isMounted) {
	...
	const createRequest = useCallback((reason: DishCategoriesTutorialOpenReason) => {
	...
	const markPresented = useCallback(() => {
		...
		void AsyncStorage.setItem(DISH_CATEGORIES_TUTORIAL_STORAGE_KEY, "true").catch((error) => {
			void logFrontendEvent({
				event_name: "topics_tutorial_storage_failed",
				error_level: "warn",
				payload: {
